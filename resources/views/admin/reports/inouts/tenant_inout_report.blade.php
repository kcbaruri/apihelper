...
<html>
   ...
   <div style="text-align: center;" >
         ...
         <hr/>
         <h2 style="color:#6f6f6f;margin:0"><span>{{ __('reports.inoutreport') }}</span></h2>
      </div>
        ...
        <th width="20%">{{ __('sidebar.flats') }}</th> 
        <th width="20%">{{ __('reports.tenant_name') }}</th> 
        <th width="20%">{{ __('pages.mobile_no') }}</th> 
        <th width="20%">{{ __('reports.in_date') }}</th>
        <th width="20%">{{ __('reports.out_date') }}</th>
        </tr>
        <?php 
        $i = 1;
        foreach($tenants as $tenant){
            ...
            <td width="65%"><?php if($tenant->floor != NULL) echo $tenant->floor->name; ?></td>
            <td width="20%"><?php if($tenant->flat != NULL) echo $tenant->flat->name; ?></td>
            <td width="65%"><?php  echo $tenant->name;?></td>
            <td width="20%"><?php echo $tenant->mobile_number;?></td>
            <td width="20%"><?php echo $tenant->in_date;?></td>
            <td width="20%"><?php echo $tenant->out_date;?></td>
            </tr>
            <?php
            $i++;
        }
